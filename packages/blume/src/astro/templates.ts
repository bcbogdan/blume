...
const renderIntegrationBridge = (
  ...
  return {
    ...
    userVitePluginSpread: ", ...(blumeConfig?.vite?.plugins ?? [])",
  };
};
...
const renderRuntimeModuleWiring = (
  ...
  const aliasLines = [...RUNTIME_MODULE_FILES]
    .map(
      ([id, file]) =>
        `\n        ${JSON.stringify(id)}: blumeFileURLToPath(new URL(${JSON.stringify(`${generatedModulesDir}/${file}`)}, import.meta.url)),`
    )
    .join("");
  return {
    aliasLines,
...
const renderRuntimeAlias = (
  path: string | undefined,
  generatedModulesDir: string | undefined
): string => {
  if (path === undefined || generatedModulesDir === undefined) {
    return JSON.stringify(path) ?? "undefined";
  }
  return `blumeFileURLToPath(new URL(${JSON.stringify(path)}, import.meta.url))`;
};
...
export const astroConfigTemplate = (options: {
  ...
  const {
    ...
    nodeImport: runtimeModuleNodeImport,
...
${configSourceMarker}${userConfigImports}${runtimeModuleNodeImport}${defineConfigImport}
...
${userConfigSetup}export default defineConfig({
  ...
  vite: {${viteCacheOption}
    define: { "import.meta.env.BLUME_ASK_ENDPOINT": ${JSON.stringify(JSON.stringify(askEndpoint(config)))} },
    plugins: [${runtimeModulesPluginEntry}tailwindcss(), includeHmrPlugin(${JSON.stringify(
      `${context.outDir}/src/generated/includes.json`
    )}), prerenderDepsPlugin()${userVitePluginSpread}],
    ...
    resolve: {
      alias: {
        "blume:ask": ${renderRuntimeAlias(askPath, generatedModulesDir)},
        "blume:examples": ${renderRuntimeAlias(examplesPath, generatedModulesDir)},
        "blume:examples-theme": ${renderRuntimeAlias(examplesThemePath, generatedModulesDir)},
        "blume:search-client": ${renderRuntimeAlias(searchClientPath, generatedModulesDir)},
        "blume:theme": ${renderRuntimeAlias(themePath, generatedModulesDir)},${runtimeModuleAliasLines}${userAliasLines}
      },
    },
    server: {
      fs: {
...
import { deployedArtifactRoute } from "blume/ai/artifact-routes.ts";
import Icon from "blume/components/Icon.astro";
