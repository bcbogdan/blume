...
import { agentArtifactFiles } from "../astro/agent-artifacts.ts";
...
import type { BlumeData } from "../core/data.ts";
...
const removeGeneratedEndpoint = async (path: string): Promise<void> => {
  if (!existsSync(path)) {
    return;
  }
...
/** Remove the former extensionless API index, which collides with the API directory in static output. */
const removeApiNamespaceFiles = async (
  project: BlumeProject,
  srcDir: string,
  genDir: string
): Promise<void> => {
  const dataPath = join(genDir, "data.json");
  const previous: BlumeData | null = existsSync(dataPath)
    ? JSON.parse(await readFile(dataPath, "utf-8"))
    : null;
  const bases = new Set([
    "",
    project.config.basePath,
    previous?.config.basePath ?? "",
  ]);
  await Promise.all(
    [...bases].map((base) =>
      removeGeneratedEndpoint(join(srcDir, "pages", base, "api/docs/index.ts"))
    )
  );
};
...
const askFiles = async (
  ...
  const previousDataPath = join(genDir, "data.json");
  const previousData: BlumeData | null = existsSync(previousDataPath)
    ? JSON.parse(await readFile(previousDataPath, "utf-8"))
    : null;
  const currentPath =
  ...
  const oldPaths = new Set([
    ...
    join(srcDir, "pages", previousData?.config.basePath ?? "", "api", "ask.ts"),
    join(srcDir, "pages", project.config.basePath, "api", "ask.ts"),
  ]);
  await Promise.all(
    [...oldPaths]
      .filter((path) => path !== currentPath)
      .map(removeGeneratedEndpoint)
  );
  ...
  const files = [
    {
      ...
      path: join(srcDir, "pages", project.config.basePath, "api", "ask.ts"),
    },
  ];
  if (grounded) {
    files.push({
...
const mcpDiscoveryPages = (
  ...
  hostsMcp(project, userPages) && project.config.ai.mcp.discovery
    ? [
        {
...
const mcpFiles = async (
  ...
  const discoveryPages = mcpDiscoveryPages(project, userPages);
  await Promise.all(
    ["discovery.ts", "server-card.ts"]
      .filter(
        (name) =>
          !discoveryPages.some((page) => page.entrypoint.endsWith(`/${name}`))
      )
      .map((name) => removeGeneratedEndpoint(join(srcDir, "blume-mcp", name)))
  );
  ...
  return [
    ...
    ...(discoveryPages.some((page) => page.entrypoint.endsWith("/discovery.ts"))
      ? [
          {
            content: staticJsonEndpointTemplate(
              buildMcpDiscovery(discoveryInput)
            ),
            path: join(srcDir, "blume-mcp", "discovery.ts"),
          },
        ]
      : []),
    ...(discoveryPages.some((page) =>
      page.entrypoint.endsWith("/server-card.ts")
    )
      ? [
          {
            content: staticJsonEndpointTemplate(
              buildMcpServerCard(discoveryInput)
            ),
...
export const eject = async (
  ...
  await removeApiNamespaceFiles(project, srcDir, genDir);
  ...
  const relPages = [
    ...artifactFiles.flatMap((file) =>
      file.pattern
        ? [
            {
              entrypoint: toPosix(relative(root, file.path)),
  ...
  if (config.ai.api) {
    ...
    await writeApiFiles(
      ...
      data,
      {
        enabled: hostsMcp(project, pages),
