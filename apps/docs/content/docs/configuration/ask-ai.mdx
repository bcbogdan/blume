...
The built-in endpoint follows `basePath`: `/docs` generates `/docs/api/ask`. With `deployment.base: "/host"`, the UI and manifest use `/host/docs/api/ask`. This also applies after eject. An explicit `ai.ask.endpoint` is used as supplied and does not generate a local endpoint.
