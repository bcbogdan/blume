...
### Stable tab values and multiple fences

Pass `tabValues` as a **JSON string** mapping fence titles to stable values. Fences with the same value become direct children of one panel, in source order. Tabs follow the first occurrence of each value and use that fence's title as their label; the panel exposes the value as `data-tab-id`.

<CodeGroup tabValues='{"Install":"node","Configure":"node","Python":"python"}'>

```sh Install
npm install example
```

```js Configure option="runtime=node"
...
Every fence title must be an own key in the mapping. Keys and values must be nonempty strings (whitespace-only strings are invalid). Invalid JSON, an invalid mapping shape, or a missing title marks the group with `data-code-group-invalid="true"` and falls back to one tab per fence. Explicit `<Tab>` panels ignore the mapping. Without `tabValues`, the existing one-tab-per-fence behavior applies.

Groups containing a multi-fence panel keep a copy button on each block. Single-fence groups use the shared tab-bar button. Copying respects hidden panels and code surfaces.

### Fence option metadata

Use a quoted `option="..."` or `option='...'` attribute after the language to attach an opaque `data-code-option` value to the rendered `<pre>`. It can coexist with a title, line highlights, and `lineNumbers`:

````md
```ts title="Setup" option="framework=next; ui=custom" lineNumbers
export const enabled = true;
```
````

Blume preserves the value but does not interpret it or hide alternatives. Your application can use it to select code surfaces. Empty, unquoted, malformed, or embedded lookalike attributes are ignored.

## Frame

Wrap an image or any visual in a centered, bordered frame with an optional `caption` (rendered as Markdown) and `hint`.

Set `borderless` (default: `false`) to remove the border, background, padding, and rounding. Media remains centered and responsive; captions, hints, and autoplay video behavior are preserved.

<Frame
  borderless
  caption="A **borderless** illustration."
  hint="No surrounding frame chrome."
>
  <svg
    width="160"
    height="72"
    viewBox="0 0 160 72"
    role="img"
    aria-label="Borderless sample"
  >
    <rect width="160" height="72" fill="#3b82f6" />
  </svg>
</Frame>

```astro
<Frame borderless caption="Product overview">
